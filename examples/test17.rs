...
fn setup_menu_scene(
    ...
    mut background_query: Query<&mut Visibility, (With<MainMenuBackground>, Without<LeftAnchoredButton>, Without<GameMenuOverlay>)>,
) {
    println!("进入主菜单场景");
    
    // 显示主菜单背景
    if let Ok(mut bg_visibility) = background_query.get_single_mut() {
