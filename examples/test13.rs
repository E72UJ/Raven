...
fn main() {
    App::new()
        ...
        .add_message::<AdvanceDialogueEvent>()
        ...
        .add_systems(
            FixedUpdate,
            (auto_play_timer_system, text_typewriter_system),
        )
        .run();
}
...
fn load_fonts(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    commands.insert_resource(FontAssets { main_font });
...
fn auto_play_timer_system(
    ...
    mut dialogue_events: MessageWriter<AdvanceDialogueEvent>,
...
fn update_dialogue_display(
    ...
    mut speaker_query: Query<
        ...
        (
            ...
            Without<AutoPlayIndicator>,
            Without<ProgressIndicator>,
        ),
    >,
    mut dialogue_query: Query<
        &mut Text,
        (
            With<DialogueText>,
            Without<SpeakerText>,
    ...
    >,
    mut auto_play_query: Query<
        (&mut Text, &mut TextColor),
        (
            With<AutoPlayIndicator>,
            Without<SpeakerText>,
            Without<DialogueText>,
            Without<ProgressIndicator>,
        ),
    ...
    if vn_state.current_dialogue_index < dialogue_data.lines.len() {
        ...
        if let Ok(mut dialogue_text) = dialogue_query.single_mut() {
            ...
            let displayed_text: String = full_text.chars().take(vn_state.chars_displayed).collect();
