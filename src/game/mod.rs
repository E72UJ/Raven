...
fn should_create_buttons(
    ...
    // 只在状态真正改变时才重新创建
    let state_changed = game_state.is_changed();
    
    // 只有在游戏状态改变且需要切换按钮状态时才运行
    state_changed && ((has_choices && !buttons_exist) || (!has_choices && buttons_exist))
}
