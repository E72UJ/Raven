...
#[derive(Message)]
pub struct RollbackEventMessage; // 新增回退事件

#[derive(Message)]
pub struct ToggleAutoPlayEventMessage; // 新增自动播放事件
...
impl Plugin for ToolbarPlugin {
    fn build(&self, app: &mut App) {
        app.add_message::<ToggleMenuEventMessage>() // 添加事件
            .add_message::<RollbackEventMessage>() // 注册回退事件
            .add_message::<ToggleAutoPlayEventMessage>() // 注册自动播放事件
...
fn cleanup_toolbar(mut commands: Commands, toolbar_query: Query<Entity, With<ToolbarContainer>>) {
...
fn handle_toolbar_buttons(
    ...
    >,
    mut toggle_menu_event: MessageWriter<ToggleMenuEventMessage>,
    mut rollback_event: MessageWriter<RollbackEventMessage>, // 添加回退事件发送器
    mut toggle_auto_play_event: MessageWriter<ToggleAutoPlayEventMessage>, // 添加自动播放事件发送器
) {
            ...
            Interaction::Pressed => {
                *color = BackgroundColor(Color::srgba(0.4, 0.4, 0.4, 0.9));
                match button_type {
                    ToolbarButton::Settings => {
                        println!("设置按钮被点击");
                        toggle_menu_event.write(ToggleMenuEventMessage);
                    }
                    ToolbarButton::Rollback => {
                        println!("回退按钮被点击");
                        println!("回退事件被发送");
                        rollback_event.write(RollbackEventMessage); // 发送回退事件
                    }
                    ToolbarButton::History => {
                        println!("历史按钮正在开发中");
                    }
                    ...
                    ToolbarButton::Auto => {
                        println!("自动按钮被点击");
                        toggle_auto_play_event.write(ToggleAutoPlayEventMessage); // 发送自动播放事件
                    }
                    ToolbarButton::Save => {
                        println!("存档按钮被点击");
                    }
...
//                 toggle_menu_event.send(ToggleMenuEventMessage);
//             }
//         }
//     }
// }
...
fn button_pressed(
    buttons: &Query<(&Interaction, &ToolbarButton)>,
    target_button: &ToolbarButton,
) -> bool {
